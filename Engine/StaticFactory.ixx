...
private:
	static std::map<identifier_type, std::shared_ptr<product_type>> prototypes;
};
...
{
	auto match = prototypes.find(identifier);
	if (match == prototypes.end())
		return nullptr;
...
{
	return prototypes.contains(type);
}